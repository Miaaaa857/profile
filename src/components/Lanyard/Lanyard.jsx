...
export default function Lanyard({ position = [0, 0, 30], gravity = [0, -40, 0], fov = 20, cardEmail = '' }) {
  ...
  return (
    <div className="lanyard-wrapper">
      <Canvas camera={{ position, fov }} dpr={[1, isMobile ? 1.35 : 1.75]} gl={{ alpha: true, antialias: !isMobile }} onCreated={({ gl }) => gl.setClearColor(new THREE.Color(0x000000), 0)}>
        <ambientLight intensity={Math.PI} />
        <Physics gravity={gravity} timeStep={isMobile ? 1 / 30 : 1 / 60}><Band isMobile={isMobile} cardEmail={cardEmail} /></Physics>
...
function Band({ isMobile, cardEmail }) {
  ...
  const cardMap = useMemo(() => {
    ...
    const x = rectX * canvas.width, y = rectY * canvas.height, width = rectWidth * canvas.width, height = rectHeight * canvas.height
    context.save(); context.translate(x, y); context.scale(width / 900, height / 1200)
    context.fillStyle = '#f5f4f0'; context.fillRect(0, 0, 900, 1200)
    context.strokeStyle = '#d8d6d0'; context.lineWidth = 3; context.beginPath(); context.roundRect(58, 58, 784, 1084, 36); context.stroke()
    ...
    context.strokeStyle = '#d8d6d0'; context.lineWidth = 3; context.beginPath(); context.roundRect(58, 58, 784, 1084, 36); context.stroke()
    context.fillStyle = '#ff5232'; context.beginPath(); context.roundRect(84, 92, 128, 42, 21); context.fill()
    context.fillStyle = '#fff'; context.font = '700 19px Arial'; context.textAlign = 'center'; context.fillText('PROFILE', 148, 120)
    if (logoTexture.image) { const maxLogoWidth = 560, maxLogoHeight = 350; const logoScale = Math.min(maxLogoWidth / logoTexture.image.width, maxLogoHeight / logoTexture.image.height); const logoWidth = logoTexture.image.width * logoScale, logoHeight = logoTexture.image.height * logoScale; context.drawImage(logoTexture.image, (900 - logoWidth) / 2, 350, logoWidth, logoHeight) }
    context.fillStyle = '#171715'; context.font = '700 62px Arial'; context.textAlign = 'center'; context.fillText("MIA'S PROFILE", 450, 870)
    context.fillStyle = '#77736d'; context.font = '20px Arial'; context.fillText('AI PRODUCT · EXPERIENCE · GROWTH', 450, 920)
    context.fillStyle = '#ff5232'; context.beginPath(); context.arc(450, 1040, 12, 0, Math.PI * 2); context.fill(); context.restore()
    const result = new THREE.CanvasTexture(canvas)
    result.colorSpace = THREE.SRGBColorSpace; result.flipY = baseMap.flipY; result.anisotropy = 16; result.needsUpdate = true
    return result
  ...
  return <>
    <group position={[0, 4, 0]}>
      ...
      <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
        ...
        <group scale={[2.374, 2.25, 2.25]} position={[0, -1.2, -0.05]} onPointerOver={() => setHovered(true)} onPointerOut={() => setHovered(false)} onPointerUp={(event) => { event.target.releasePointerCapture(event.pointerId); setDragged(false) }} onPointerDown={(event) => { event.stopPropagation(); event.target.setPointerCapture(event.pointerId); setDragged(new THREE.Vector3().copy(event.point).sub(vec.copy(card.current.translation()))) }}>
          <mesh geometry={nodes.card.geometry}><meshPhysicalMaterial map={cardMap} map-anisotropy={16} clearcoat={isMobile ? 0 : 1} clearcoatRoughness={0.15} roughness={0.9} metalness={0.35} /></mesh>
          <mesh geometry={nodes.clip.geometry} material={materials.metal} material-roughness={0.3} />
          <mesh geometry={nodes.clamp.geometry} material={materials.metal} />
        </group>
      </RigidBody>
    </group>
    <mesh ref={band}><meshLineGeometry /><meshLineMaterial color="white" depthTest={false} resolution={isMobile ? [1000, 2000] : [1000, 1000]} useMap map={texture} repeat={[-4, 1]} lineWidth={1} /></mesh>
  </>
}
