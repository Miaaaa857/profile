...
export default function Prism({
  ...
  useEffect(() => {
    ...
    const pointerMove = (event) => {
      targetYaw = -((event.clientX / Math.max(1, window.innerWidth)) * 2 - 1) * 0.6 * Math.max(0, hoverStrength)
      targetPitch = ((event.clientY / Math.max(1, window.innerHeight)) * 2 - 1) * 0.6 * Math.max(0, hoverStrength)
    }
    if (animationType === 'hover') window.addEventListener('pointermove', pointerMove, { passive: true })
    ...
    const render = (now) => {
      ...
      if (animationType === 'hover') {
        yaw += (targetYaw - yaw) * Math.max(0, Math.min(1, inertia))
        pitch += (targetPitch - pitch) * Math.max(0, Math.min(1, inertia))
        setRotation(yaw, pitch, 0)
      } else if (animationType === '3drotate') {
        const t = elapsed * Math.max(0, timeScale)
        setRotation(t * 0.45, Math.sin(t * 0.6) * 0.6, Math.sin(t * 0.3) * 0.5)
      }
