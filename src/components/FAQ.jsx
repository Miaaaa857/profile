  ...
  return (
    <section className="section faq container">
      ...
      <div className="faq-list">
        {items.map((item, i) => <div className={`faq-item cursor-target ${open === i ? 'is-open' : ''}`} key={item.q}><button onClick={() => setOpen(open === i ? -1 : i)} aria-expanded={open === i}><strong>{item.q}</strong><i>{open === i ? '−' : '+'}</i></button><div className="faq-answer"><p>{item.a}</p></div></div>)}
      </div>
    </section>
  )
}
